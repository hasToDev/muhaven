...
// Wave 5 Slice 2c follow-up WS-A — scoped-session mirror wire types +
// transform hoisted to a shared module so the keyless reinvest runner
// (`reinvest/execute.ts`) can self-heal from the mirror without importing
// this tool surface. The private `syncSnapshotFromMirror` below still
// lives here and calls `mirrorDtoToPolicySnapshot`.
import {
  type ScopedSessionMirrorDto,
...
// Wave 5 Slice 2c follow-up WS-A — `ScopedSessionMirrorDto` +
// `ScopedSessionMirrorResponse` were hoisted to
// `../clients/scoped-session-mirror.js` (imported above) so the keyless
// reinvest runner reuses the exact same wire types + transform without
// importing this tool surface.
