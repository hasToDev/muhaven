...
export interface UserOperationReceipt {
  ...
  /**
   * Wave 5 Slice 1 (MCP sell) — the logs generated by THIS UserOp ONLY
   * (ERC-4337 `eth_getUserOperationReceipt.logs`, distinct from the whole-tx
   * `receipt.logs`). This is the correct source for attributing a
   * `QueueSubmitted.requestId` to this op — it can't pick up another user's
   * event under multi-UserOp bundling. `undefined` when the bundler omits it
   * (then the sell path reports `settlement:'instant'`/`requestId:null` —
   * degraded but never WRONG). Same trimmed shape as `receipt.logs`.
   */
 ...
 */
/**
 * Wave 5 Slice 1 (MCP sell) — normalize a raw `logs` array (from either the
 * top-level per-UserOp `logs` or the whole-tx `receipt.logs`) down to the
 * three fields the `QueueSubmitted` / `SelectorSet` topic-matches read.
 * Tolerates absence + non-array shape (returns `undefined`) and skips
 * non-object entries. Pure.
 */
function normalizeReceiptLogs(
  raw: unknown,
): { address?: string; topics?: string[]; data?: string }[] | undefined {
  if (!Array.isArray(raw)) return undefined;
  return raw
    .filter((l): l is Record<string, unknown> => typeof l === 'object' && l !== null)
    .map((l) => ({
      ...(typeof l.address === 'string' ? { address: l.address } : {}),
      ...(Array.isArray(l.topics) && l.topics.every((t) => typeof t === 'string')
...
function parseReceipt(raw: unknown): UserOperationReceipt {
  ...
  // Wave 5 Slice 1 (MCP sell) — preserve event logs so the sell path can
  // parse `QueueSubmitted` for the queue requestId. Per ERC-4337,
  // `eth_getUserOperationReceipt` returns TWO log arrays:
  //   - top-level `logs`   = the logs generated by THIS UserOp only.
  //   - `receipt.logs`     = the WHOLE bundled transaction's logs.
  // For attributing a requestId to THIS op we MUST use the top-level
  // per-UserOp `logs` — `receipt.logs` could, under multi-UserOp bundling,
  // contain another user's `QueueSubmitted` and misattribute its requestId.
  // We keep `receipt.logs` too (emitter-filtered consumers like the
  // MODE.ENABLE SelectorSet search rely on it). (CR review HIGH +
  // self-review 2026-05-25.)
  const topLevelLogs = normalizeReceiptLogs(obj.logs);
  const txLogs = normalizeReceiptLogs(innerObj.logs);
  ...
  return {
    ...
    ...(topLevelLogs ? { logs: topLevelLogs } : {}),
    receipt: {
