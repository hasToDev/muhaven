...
test('issuer tokens page renders aggregate stats + token cards', async ({
  ...
  // Either at least one token card OR the empty state.
  const tokenCard = page.locator('p:has-text("FHE Encrypted")').first()
  const emptyState = page.getByText(/No tokens issued yet/i)

  await Promise.race([
    tokenCard.waitFor({ state: 'visible', timeout: 15_000 }),
