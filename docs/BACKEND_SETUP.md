...
> Docker Compose stack (postgres + backend + fhe-worker + nav-worker), env configuration, deploy flow, and Cloudflare tunnel setup.

For day-to-day homelab operation see the internal `development/DEV_WAVE_3/HOMELAB_DEPLOY.md`. This doc is the canonical reader-facing guide.
...
|---------|------|------|-----------|
| `postgres:16-alpine` | 5432 | Drizzle schema store: users, sessions, nonces, escrows, withdrawals, rwa_tokens, portfolios, yield_records, escrow_events, nav_history | — |
| `backend` | 3000 | REST API: auth, portfolio, yields, deposit/withdraw, issuer, webhooks | postgres (healthy), fhe-worker (started) |
...
Only port 3000 is exposed publicly via the Cloudflare tunnel. All intra-stack traffic uses Docker DNS names (`postgres`, `fhe-worker`, `nav-worker`).
...
**Contract addresses** (populated from `deployments/arb-sepolia.json` after deploy)

| Var | Contract |
|-----|----------|
| `MUHAVEN_TOKEN_ADDRESS` | fhERC-20 proxy |
| `MUHAVEN_VAULT_ADDRESS` | ERC-20 ↔ fhERC-20 wrapper proxy |
| `REINEIRA_ESCROW_ADDRESS` | MuHaven's two-phase escrow proxy. **Note:** the variable is called `REINEIRA_ESCROW_ADDRESS` for backwards compatibility with the ReineiraOS starter. The deployed contract at this address is `MuHavenEscrow`, not ReineiraOS's `ConfidentialEscrow`. |
| `INVESTOR_REGISTRY_ADDRESS` | Holder enumeration proxy |
| `YIELD_DISTRIBUTOR_ADDRESS` | Distribution state machine proxy |
| `YIELD_GATE_ADDRESS` | Condition resolver (standalone) |
| `KYC_ADAPTER_ADDRESS` | ERC-3643 KYC adapter (standalone) |
| `RISK_PARAMS_ADDRESS` | Encrypted risk guardrails proxy |
| `PUSDC_ADDRESS` | ReineiraOS ConfidentialUSDC |
| `CIRCLE_USDC_ADDRESS` | Circle USDC on Arb Sepolia |
| `SIMPLE_CONDITION_ADDRESS` | ReineiraOS gate condition base (optional) |
...
```bash
pnpm run deploy:homelab
```

This script (`scripts/deploy-homelab.sh`):

1. `scp`s changed files (`backend/`, `fhe-worker/`, `nav-worker/`, `docker-compose.yml`) to the homelab. Excludes `node_modules/`, `dist/`, `.env`, `.git/` via each service's `.dockerignore`.
2. SSHes into the homelab and runs `docker compose up -d --build --no-deps backend fhe-worker nav-worker` — postgres is never restarted.
3. Waits for `/health` on `http://localhost:3000/health` to return 200 before exiting.
...
cp nav-worker/.env.example nav-worker/.env      && edit nav-worker/.env  # if present
...
Then from the **dev machine** (not inside a container — `drizzle-kit` needs Node + schema):
...
Expected output: `[✓] Changes applied: Created table users ...` across 11 tables and 5 enums (`escrow_status`, `withdrawal_status`, `wallet_provider`, `user_role`, `escrow_event_type`, `asset_class`, `yield_status`).
...
The schema lives in `backend/drizzle/schema.ts`. Drizzle Kit reads `DATABASE_URL` from the environment — it does not parse `.env` files, so set it inline.

| Command | When to use |
|---------|-------------|
| `pnpm db:push` | Idempotent schema push. Safe for testnet. First-run and dev iterations. |
| `pnpm db:generate` | Generate a SQL migration file after editing `schema.ts` |
| `pnpm db:migrate` | Apply SQL migrations (for production-grade schema history) |
