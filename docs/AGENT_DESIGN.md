...
> ⚠️ **This doc reflects the pre-Wave-4-research design (single backend agent, six tools, no MCP / OpenClaw / hosted checkout, no tiered autonomy).** A 2026-04-27 deep-research pass expanded Wave 4 scope to four agentic surfaces (in-dashboard HavenBot, `@muhaven/mcp` MCPB server, OpenClaw skill bundling VibeKit MCP, hosted checkout `pay.muhaven.app`) plus a tiered-autonomy engine (Advisory / Confirm-per-action / Policy-bound) and a hybrid policy-storage model (encrypted thresholds in `RiskParams.sol` + plaintext rule-shape in `@zerodev/permissions` validators). **Canonical Wave 4 plan is now `development/WAVE_PLAN.md` §"Wave 4" + `development/DEV_WAVE_4/PLAN.md`.** The supporting research artifacts are at `development/research-docs/WAVE_4_AGENTIC_RESEARCH_BRIEF.md` + `WAVE_4_AGENTIC_RESEARCH_RESULT.md`. This doc will be rewritten as part of the post-Wave-4 docs refresh; treat sections below as historical context, not current spec.
