...
**Wave 5 Path D Slice 1 update (2026-05-22).** MCP autonomous-buy mode lands on `master` (commits `7e004dc` → `fe839f0` → `7c293db`). The 5th tiered-autonomy posture — **Scoped** — sits above PolicyBound: user mints a session-key kernel scoped to `subscription.purchase` with per-op `maxSharesHint` cap; subsequent `muhaven.position.buy` calls bypass the dashboard deep-link and submit autonomously via broker-signed UserOp + bundler. Pipeline: backend `/api/v1/agent/path-d/encrypt-shares` (with hard `setAccount(kernelAddress)` to satisfy the on-chain FHE verifier) → MCP composes kernel.execute calldata → bundler `pm_sponsorUserOperation` → broker EIP-191 signs the userOpHash (policy-gated against per-session snapshot's selectorCaps) → MCP submits via ERC-4337 `eth_sendUserOperation`. Falls back to Path C dashboard deep-link with 14 distinct structured `pathDFallbackReason` codes at every gate failure. Lethal-trifecta isolation preserved: broker is zero-egress (signs hashes only); MCP server speaks to the bundler. Slice 1 is unblocked but END-TO-END requires Slice 2's frontend `storePolicySnapshot` wire-up to populate the snapshot's `permissionId` field — until then Path D returns `no_permission_id_in_snapshot` cleanly.
