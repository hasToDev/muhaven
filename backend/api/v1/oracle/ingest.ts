...
/**
 * POST /api/v1/oracle/ingest — Wave 5 Q1 RWA oracle ingest.
 *
 ...
 * tickers. Within an asset, all three writes share a transaction so
 * partial commits can't happen.
 */

// Lazy singleton — building the use case at module load eagerly opens
// the Postgres pool via `container.oracleRepo` even when the request
// is rejected by `withCors`/method check. Deferring keeps cold-start
// path clean and lets tests stub the container per-request.
let _useCase: IngestOracleUseCase | null = null;
function getUseCase(): IngestOracleUseCase {
  if (!_useCase) {
    _useCase = new IngestOracleUseCase(container.oracleRepo);
  }
  return _useCase;
}
...
const handler = createHandler({
  ...
  execute: async (dto) => {
    const result = await getUseCase().execute(dto.assets);
